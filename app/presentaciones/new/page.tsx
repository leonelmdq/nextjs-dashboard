...
const NuevaPresentacion: React.FC = () => {
    const [nombre, setNombre] = useState('');
    const [kilos, setKilos] = useState('');
    const [cantidadPorPallet, setCantidadPorPallet] = useState('');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const presentacion = { nombre, kilos, cantidad_x_pallet: cantidadPorPallet};
        ...
        try {
            ...
            if (response.ok) {
                setSuccess('Presentacion agregado exitosamente.');
                Swal.fire(
                    'Creado!',
                    'La nueva presentacion ha sido creado.',
                    'success'
                );
                ...
                Swal.fire(
                    'Error!',
                    'No se pudo crear la presentacion.',
                    'error'
                );
            } else {
                ...
                setError('Error al agregar la presentacion.');
            }
        } catch (error) {
    ...
    return (
        <div className="p-8">
            <form onSubmit={handleSubmit} className="bg-sky-600 text-white p-6 rounded shadow-md max-w-md mx-auto">
                <h1 className="text-xl font-semibold mb-4">Agregar Nueva Presentacion</h1>
                ...
                <div className="mb-4">
                    <label htmlFor="cantidadPorPallet" className="block text-sm font-medium text-white-700">Cantidad Por Pallet</label>
                    <input
                        type="text"
                        id="cantidadPorPallet"
                        value={cantidadPorPallet}
                        onChange={(e) => setCantidadPorPallet(e.target.value)}
                        className="text-black mt-1 p-2 w-full border border-gray-300 rounded"
                        required
                    />
                </div>
